...
const Template4 = ({ publicData, isPublicView }) => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0B1D33] flex items-center justify-center text-white font-sans">
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 border-4 border-[#FF6B35] border-t-transparent rounded-full animate-spin"></div>
          <p className="text-gray-400 tracking-wider text-sm uppercase">Loading Schematic...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0B1D33] text-white font-sans selection:bg-[#FF6B35] selection:text-[#0B1D33]">
      {!portfolioId && !location.state && !isPublicView && (
        <div className="bg-[#FF6B35]/20 border-b border-[#FF6B35]/40 text-[#FF6B35] px-4 py-2 text-center text-xs font-semibold tracking-wide">
