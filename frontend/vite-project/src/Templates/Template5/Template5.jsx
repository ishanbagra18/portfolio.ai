...
const Template5 = ({ publicData, isPublicView }) => {
  ...
  useEffect(() => {
    ...
    link.rel = 'stylesheet';
    link.href =
      'https://fonts.googleapis.com/css2?family=Archivo+Black&family=Space+Grotesk:wght@500;600;700&family=Space+Mono:wght@400;700&family=Inter:wght@400;500;600&display=swap';
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F6F1E4] flex items-center justify-center text-[#12110F] font-sans">
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 border-4 border-[#FF4D9D] border-t-transparent rounded-full animate-spin"></div>
          <p className="text-gray-700 font-bold tracking-wider text-sm uppercase">Loading Scrapbook...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F6F1E4] text-[#12110F] font-sans selection:bg-[#FF4D9D] selection:text-white">
      {!portfolioId && !location.state && !isPublicView && (
        <div className="bg-[#FF4D9D]/20 border-b-2 border-black text-[#12110F] px-4 py-2 text-center text-xs font-black uppercase tracking-wider">
