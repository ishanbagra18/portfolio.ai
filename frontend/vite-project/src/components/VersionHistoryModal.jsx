    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
        <motion.div
          ...
          className="relative w-full max-w-xl bg-zinc-900 border border-white/10 rounded-3xl p-6 sm:p-8 text-[var(--neo-text)] shadow-2xl font-sans"
        >
          {/* Header */}
          <div className="flex items-center justify-between pb-4 mb-6 border-b border-white/10">
            <div className="flex items-center gap-3">
              <History className="w-6 h-6 text-pink-400" />
              <h2 className="text-xl font-bold uppercase tracking-tight">Version History & Snapshots</h2>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSaveVersion} className="mb-6 flex gap-3">
            <input
              type="text"
              placeholder="e.g. V2.0 Before Tech Review"
              value={versionName}
              onChange={(e) => setVersionName(e.target.value)}
              className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-pink-400 transition"
            />
            <button
              type="submit"
              disabled={saving || !versionName.trim()}
              className="flex items-center gap-2 px-5 py-2.5 bg-pink-500 hover:bg-pink-600 disabled:opacity-40 text-white font-bold text-sm rounded-xl transition shadow-lg"
            >
