...
const PortfolioMatcher = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/ai/match-portfolio-job/${id}`, {
        method: 'POST',
        headers,
      ...
      const result = await res.json();
      if (res.ok && result.success && result.report) setReport(result.report);
      else setError(result.message || "Failed to analyze Job Match.");
  ...
  return (
    <div className="min-h-screen bg-[var(--neo-bg)] text-[var(--neo-text)] font-sans relative overflow-x-hidden pb-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 py-8 sm:py-16 relative z-10">
        ...
        <Link
          to="/my-portfolios"
          className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-[var(--neo-text)] uppercase tracking-wider mb-8 transition-colors"
        >
          &larr; Back to Portfolios
        </Link>

        {/* Header Title */}
        <div className="border-b border-black/5 dark:border-white/5 pb-8 mb-12">
          ...
          <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tighter leading-none text-[var(--neo-text)]">
            Portfolio Job Matcher
          </h1>
          <p className="text-slate-400 text-sm md:text-base mt-2">
            Analyze how well this saved portfolio matches a target job description.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-black/5 dark:bg-white/5 backdrop-blur-md border border-black/5 dark:border-white/5 rounded-2xl p-6 sm:p-8 backdrop-blur sticky top-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="flex flex-col">
                  <textarea
                    ...
                    rows="15"
                    disabled={loading}
                    className="bg-[var(--neo-bg)]/60 border border-black/10 dark:border-black/10 dark:border-white/10 focus:border-violet-500/50 text-slate-200 p-4 rounded-xl text-sm focus:outline-none placeholder:text-slate-600 transition resize-none leading-relaxed"
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3.5 bg-violet-600 hover:bg-violet-500 disabled:opacity-50 text-[var(--neo-text)] font-bold text-xs uppercase tracking-widest rounded-xl transition-all cursor-pointer shadow-lg shadow-violet-600/20 flex items-center justify-center gap-2"
                >
                  {loading ? (
                    <>
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            {loading && (
              <div className="space-y-6">
                <div className="p-6 bg-black/5 dark:bg-white/5 backdrop-blur-sm border border-black/5 dark:border-white/5 rounded-2xl animate-pulse flex items-center gap-4">
                  <div className="w-16 h-16 bg-[var(--neo-bg)] rounded-full"></div>
                  <div className="flex-1 space-y-2">
                    <div className="h-4 bg-[var(--neo-bg)] rounded w-1/3"></div>
                    <div className="h-3 bg-[var(--neo-bg)] rounded w-1/2"></div>
                  </div>
                </div>
                <div className="p-6 bg-black/5 dark:bg-white/5 backdrop-blur-sm border border-black/5 dark:border-white/5 rounded-2xl animate-pulse space-y-3">
                  <div className="h-4 bg-[var(--neo-bg)] rounded w-1/4"></div>
                  <div className="h-3 bg-[var(--neo-bg)] rounded w-full"></div>
                  <div className="h-3 bg-[var(--neo-bg)] rounded w-5/6"></div>
                </div>
              </div>
            )}
            ...
            {!loading && !error && !report && (
              <EmptyState title="No Match Report" desc="Paste the JD and click Analyze Job Match to see your score." icon="📊" />
            )}
            ...
            {!loading && report && (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-300">
                  ...
                  <div className="text-center sm:text-left">
                    ...
                    <p className="text-slate-400 text-xs sm:text-sm leading-relaxed">
                      {report.matchPercentage >= 75
                        ? "Excellent alignment! Your portfolio demonstrates strong keyword matching with this job specification."
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="bg-black/5 dark:bg-white/5 backdrop-blur-md border border-black/5 dark:border-white/5 rounded-2xl p-6">
                      ...
                      <ul className="space-y-3">
                        {report.strengths.map((str, idx) => (
                          <li key={idx} className="text-slate-300 text-xs sm:text-sm leading-relaxed flex items-start gap-2">
                            <span className="text-emerald-500 font-bold mt-0.5">•</span><span>{str}</span>
                          </li>
                        ))}
                      </ul>
                    ) : (
                ...
                <div className="bg-black/5 dark:bg-white/5 backdrop-blur-md border border-black/5 dark:border-white/5 rounded-3xl p-6 sm:p-8">
                  ...
                  {report.recommendations?.length > 0 ? (
                    <div className="space-y-4">
                      {report.recommendations.map((rec, idx) => (
                        <div key={idx} className="p-4 bg-[var(--neo-bg)]/40 border border-black/10 dark:border-black/10 dark:border-white/10/80 rounded-xl text-slate-300 text-xs sm:text-sm leading-relaxed flex gap-3">
...
const EmptyState = ({ title, desc, icon }) => (
  <div className="bg-[var(--neo-bg)]/10 border border-dashed border-black/5 dark:border-white/5 rounded-3xl p-16 text-center">
    <div className="w-16 h-16 bg-black/5 dark:bg-white/5 backdrop-blur-md border border-black/10 dark:border-black/10 dark:border-white/10 rounded-2xl flex items-center justify-center mx-auto mb-6 text-3xl">
      {icon}
    </div>
...
export default PortfolioMatcher;
